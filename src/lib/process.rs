...
fn print_push<S: Into<String>>(msgs: &mut Vec<String>, msg: S) {
    msgs.push(msg.into());
}
...
pub fn process(conf_file_path: &str) -> Vec<String> {
    ...
    for dict_conf in config.dictionary_inputs {
        print_push(&mut result, format!("Lendo {}", dict_conf));
    ...
    for file_conf in config.template_files {
        print_push(&mut result, format!("Processando {}", file_conf.name));
